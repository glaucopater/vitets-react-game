...
const App: React.FC = () => {
  ...
  const [respawnKey, setRespawnKey] = useState(0); // Add a key to trigger respawn logic
  ...
  const resetGame = () => {
    ...
    setEnemies([]);
    setIsGameOver(false);
    setRespawnKey((prevKey) => prevKey + 1); // Update respawn key to trigger respawn logic
  };
  ...
  useEffect(() => {
    ...
    return () => {
      setEnemies([]); // Clear enemies when component unmounts
    };
  }, [respawnKey]); // Trigger respawn logic when respawnKey changes
  ...
  useEffect(() => {
    if (enemies.length === 0) setRespawnKey((prevKey) => prevKey + 1); // Update respawn key to trigger respawn logic
  }, [enemies.length]);
