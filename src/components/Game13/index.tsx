...
const App: React.FC = () => {
  ...
  const handleMouseClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Spawn bullet with initial position of the player
    const bulletSpeed = 0.5; // Adjust bullet speed as needed
    setBullets((prevBullets) => [
      ...prevBullets,
      { x: position.x, y: position.y, angle, speed: bulletSpeed },
    ]);
  };
      ...
      if (!isGameOver) {
        ...
        setBullets((prevBullets) =>
          prevBullets.map((bullet) => ({
            x: bullet.x + bullet.speed * Math.cos(bullet.angle),
            y: bullet.y + bullet.speed * Math.sin(bullet.angle),
            angle: bullet.angle,
            speed: bullet.speed,
          }))
        );
